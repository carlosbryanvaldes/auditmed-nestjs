"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.MasterController = void 0;
const common_1 = require("@nestjs/common");
const master_service_1 = require("./master.service");
const jwt_auth_guard_1 = require("../auth/guards/jwt-auth.guard");
const permissions_guard_1 = require("../common/guards/permissions.guard");
const permissions_decorator_1 = require("../common/decorators/permissions.decorator");
let MasterController = class MasterController {
    constructor(masterService) {
        this.masterService = masterService;
    }
    findAll() {
        return this.masterService.findAllUnidades();
    }
    create(body) {
        return this.masterService.createUnidad(body);
    }
    update(id, body) {
        return this.masterService.updateUnidad(id, body);
    }
    remove(id) {
        return this.masterService.deleteUnidad(id);
    }
};
exports.MasterController = MasterController;
__decorate([
    (0, common_1.Get)('unidades-ejecutoras'),
    (0, permissions_decorator_1.RequirePermissions)('master:read'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], MasterController.prototype, "findAll", null);
__decorate([
    (0, common_1.Post)('unidades-ejecutoras'),
    (0, permissions_decorator_1.RequirePermissions)('master:write'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], MasterController.prototype, "create", null);
__decorate([
    (0, common_1.Put)('unidades-ejecutoras/:id'),
    (0, permissions_decorator_1.RequirePermissions)('master:write'),
    __param(0, (0, common_1.Param)('id', common_1.ParseIntPipe)),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, Object]),
    __metadata("design:returntype", void 0)
], MasterController.prototype, "update", null);
__decorate([
    (0, common_1.Delete)('unidades-ejecutoras/:id'),
    (0, permissions_decorator_1.RequirePermissions)('master:write'),
    __param(0, (0, common_1.Param)('id', common_1.ParseIntPipe)),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", void 0)
], MasterController.prototype, "remove", null);
exports.MasterController = MasterController = __decorate([
    (0, common_1.Controller)('master'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard, permissions_guard_1.PermissionsGuard),
    __metadata("design:paramtypes", [master_service_1.MasterService])
], MasterController);
//# sourceMappingURL=master.controller.js.map